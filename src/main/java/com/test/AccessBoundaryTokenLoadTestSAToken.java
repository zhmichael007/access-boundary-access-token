...
class MyThreadTest extends Thread {
    ...
    public void run() {
        ...
        while (i < num) {
            try {
                i++;
                String token = AccessBoundaryToken.RefreshServiceAccountToken(43200);
                System.out.println("refresh service account token");
                if (i == 1 || i == num) {
                    System.out.println(token);
                }
                System.out.println(Thread.currentThread().getName().toString() + " Token Number: " + i);
            } catch (Exception e) {
                e.printStackTrace();
                break;
            }
        ...
        System.out.println(Thread.currentThread().getName().toString() + ": Time Used: " + (d2 - d1) / 1000
                + " Start Time: " + d1 + ", Current time: " + d2);
    }

}

/** Demonstrates how to use Downscoping with Credential Access Boundaries. */
public class AccessBoundaryTokenLoadTestSAToken {
    public static void main(String[] args) throws IOException {
        ...
        timer.scheduleAtFixedRate(ServiceAccountTokenRefreshTask, Calendar.getInstance().getTime(),
